...
%         imagesc(im1); hold on; 
%         line([1   size(im1,2)], [size(im1,1) /2 size(im1,1) /2], 'Color', 'white'); 
%         hold on;
%         colormap(gray);
%         
%         for j=1:length(pty2)
%             if(labels{patient}(pty2(j),ptx2(j),slice))
%                 plot( ptx2(j), pty2(j), 'wo');
%             else
%                 plot( ptx2(j), pty2(j), 'ro');
%             end
%         end
%             
%         close;
      ...
      %  cb2 = extractCuboidNoReshape(flip(im1,1), pty2, ptx2, winSize);
        
   
        X1{i} = cat(3, X1{i}, cb1);
%        X2{patient} = cat(3, X2{patient}, cb2);
