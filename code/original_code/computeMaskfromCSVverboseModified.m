...
list = dir(fullfile(rootCSV, '*.csv'))
for i=1:length(list) % #loops = #csv files
    if eq(i,17) | eq(i,42)
        continue
    end
    
    fnameCsvAdc = fullfile(rootCSV, list(i).name)
    
    listDicom = subdir(fullfile(rootCSV, num2str(i), '*.dcm'));
